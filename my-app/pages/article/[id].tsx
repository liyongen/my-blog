...
const ArticleDetail = (props: IProps) => {
  ...
  const {
    user: { nickname, avatar, id },//解释这个user的结构
  } = article;
  const [inputVal, setInputVal] = useState('');
  const [comments, setComments] = useState(article?.comments || []);

  console.log(22222);
  console.log(article);//打印在浏览器上
    ...
    <div>
      ...
      <div className="content-layout">
        <div className={styles.comment}>
          <h3>评论</h3>
          {/* loginUserInfo?.userId &&这种写法就是说已经登陆的情况下执行下面的html */}
...
export default observer(ArticleDetail);//整个都是ssr页面
